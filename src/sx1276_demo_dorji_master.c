...
#define RegFreqMid 0x07 //Freq Middle bit
...
#define RegOsc 0x24 //OSC setting
...
**Function: Set sx1276 entry LoRa(LongRange) mode
**Input: None
**Output: None
**********************************************************/
...
SPIWrite(REG_LR_DIOMAPPING1,0x01); //DIO0=00, DIO1=00,
DIO2=00, DIO3=01
SPIWrite(REG_LR_DIOMAPPING2,0x40); //DIO4 01
SPIWrite(LR_RegIrqFlagsMask,0x3F); //Open RxDone interrupt &
Timeout
sx1276_7_8_LoRaClearIrq();
SPIWrite(LR_RegPayloadLength,21); //RegPayloadLength
21byte(this register must difine when the data long of one byte in SF is 6)
addr = SPIRead(LR_RegFifoRxBaseAddr); //Read
RxBaseAddr
SPIWrite(LR_RegFifoAddrPtr,addr); //RxBaseAddr -> FiFoAddrPtr
//SPIWrite(LR_RegOpMode,0x8d); //Continuous Rx
Mode//Low Frequency Mode
SPIWrite(LR_RegOpMode,0x85); //Continuous Rx
...
temp=temp+127-137; //127:Max RSSI, 137:RSSI
offset
...
SPIWrite(LR_RegFifoAddrPtr,addr); //RxBaseAddr -> FiFoAddrPtr
if(sx1276_7_8SpreadFactorTbl[Lora_Rate_Sel]==6) //When SpreadFactor is six，
will used Implicit Header mode(Excluding internal packet length)
packet_size=21;
else
...
SPIWrite(LR_RegHopPeriod,0x00); //RegHopPeriod NO
FHSS
SPIWrite(REG_LR_DIOMAPPING1,0x41); //DIO0=01, DIO1=00,
DIO2=00, DIO3=01
SPIWrite(REG_LR_DIOMAPPING2,0x00); //DIO4 00
sx1276_7_8_LoRaClearIrq();
...
//SPIWrite(LR_RegOpMode,0x8b); //Tx Mode Low Frequency Mode
SPIWrite(LR_RegOpMode,0x83); //Tx Mode High Frequency Mode
while(1)
{
...
{
unsigned char i;
sx1276_7_8_Sleep(); //Change modem mode Must
in Sleep mode
...
SPIWrite(LR_RegPaConfig,sx1276_7_8PowerTbl[Power_Sel]); //Setting output
power parameter
...
;//Implicit Enable CRC Enable(0x02) & Error Coding rate 4/5(0x01), 4/6(0x02), 4/7(0x03),
4/8(0x04)
SPIWrite(LR_RegModemConfig2,((sx1276_7_8SpreadFactorTbl[Lora_Rate_Sel]<<4)+(CRC<<2)
+0x03));
tmp = SPIRead(0x31);
tmp &= 0xF8;
tmp |= 0x05;
...
;//Explicit Enable CRC Enable(0x02) & Error Coding rate 4/5(0x01), 4/6(0x02), 4/7(0x03),
4/8(0x04)
SPIWrite(LR_RegModemConfig2,((sx1276_7_8SpreadFactorTbl[Lora_Rate_Sel]<<4)+(CRC<<2)
...
SPIWrite(LR_RegPreambleLsb,12); //RegPreambleLsb 8+4=12byte
Preamble
SPIWrite(REG_LR_DIOMAPPING2,0x01); //RegDioMapping2 DIO5=00,
DIO4=01
sx1276_7_8_Standby(); //Entry standby mode
}
void setup() {
